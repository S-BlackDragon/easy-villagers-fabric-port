...
public class VillagerBlockBase extends HorizontalRotatableBlock {
    ...
    private static final VoxelShape SHAPE = Shapes.or(
            Shapes.or(Block.box(0D, 0D, 0D, 16D, 1D, 16D), Block.box(0D, 15D, 0D, 16D, 16D, 16D)),
            Shapes.or(Block.box(0D, 0D, 0D, 1D, 16D, 16D), Block.box(15D, 0D, 0D, 16D, 16D, 16D)),
            Shapes.or(Block.box(0D, 0D, 0D, 16D, 16D, 1D), Block.box(0D, 0D, 15D, 16D, 16D, 16D))
    );
    ...
    public static void playRandomVillagerSound(Level world, BlockPos pos, SoundEvent soundEvent) {
        if (world.getGameTime() % Main.SERVER_CONFIG.villagerSoundAmount == 0 && world.random.nextInt(40) == 0) {
            playVillagerSound(world, pos, soundEvent);
        }
    }

    public static void playRandomVillagerSound(Player player, SoundEvent soundEvent) {
        if (player.level().getGameTime() % Main.SERVER_CONFIG.villagerSoundAmount == 0 && player.level().random.nextInt(40) == 0) {
    ...
    public static void playVillagerSound(Level world, BlockPos pos, SoundEvent soundEvent, float pitch) {
        world.playSound(null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, soundEvent, SoundSource.BLOCKS, 1F, pitch);
    }
